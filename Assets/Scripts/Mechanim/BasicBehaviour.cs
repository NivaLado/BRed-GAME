...
public class BasicBehaviour : MonoBehaviour
{
	...
	public bool IsGrounded()
	{
        ...
        return Physics.SphereCast(ray, colExtents.x, colExtents.x + 0.4f); //ray rad max
    }

    //private void OnDrawGizmosSelected()
    //{
    //    Gizmos.color = Color.cyan;
    //    Debug.DrawLine(this.transform.position, this.transform.position + Vector3.down * 0.25f);
    //}
    ...
    public float DistanceToGround()
    {
        RaycastHit hit;
        Ray ray = new Ray(this.transform.position + Vector3.up * 2 * colExtents.x, Vector3.down);
        Physics.SphereCast(ray, colExtents.x, out hit, colExtents.x + 1f); //Only 1f long ray
        return hit.distance;
    }
}
